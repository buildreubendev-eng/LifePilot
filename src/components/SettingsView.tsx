  ...
  return (
    <div>
      <section className="py-4">
        <h1 className="text-4xl font-black text-white">Preferences & Privacy</h1>
        ...
        <div className="rounded-lg border border-white/10 bg-black/40 p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3">
            {sensitiveCategories.map((category) => (
              <label key={category} className="flex items-center justify-between rounded-lg bg-black/20 p-3">
          ...
          <button type="button" onClick={() => void handleReset()} className="mt-5 rounded-md bg-emerald-600 px-4 py-3 text-sm font-semibold text-white hover:bg-stone-800">
...
function SettingRow({ label, enabled }: { label: string; enabled: boolean }) {
  return (
    <div className="flex items-center justify-between rounded-lg bg-black/20 p-3">
      ...
      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${enabled ? "bg-emerald-100 text-emerald-800" : "bg-white/10 text-stone-300"}`}>
